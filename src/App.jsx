    ...
    <div className="auth-split" style={{ display: "flex", minHeight: "100vh", fontFamily: F }}>
      ...
      <div className="auth-right" style={{ flex: "0 0 40%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 48px", background: "#0E1420", borderLeft: `1px solid rgba(255,255,255,0.04)`, overflowY: "auto" }}>
        ...
        <div style={{ marginTop: 28, padding: 16, background: T.bg, border: "1px solid rgba(255,255,255,0.04)", borderRadius: 10 }}>
          <div style={{ fontSize: 13, color: T.s, lineHeight: 1.6 }}>Start your <strong style={{ color: T.a, fontWeight: 600 }}>7-day free trial</strong>. Full Recruiter access. Cancel anytime before day 7 — no charge.</div>
        </div>
      </div>
  ...
  return (<>
    <div style={{ minHeight: "100vh", background: T.bg, color: T.t, fontFamily: "'SF Pro Display',-apple-system,sans-serif", display: "flex", position: "relative", paddingTop: impersonating ? 42 : 0 }}>
      ...
      <div className="main-scroll" style={{ flex: 1, overflow: "auto", padding: (view === "lead" || view === "addlead") ? "0 0 80px 0" : "24px 32px 80px 32px" }}>
        ...
        {view === "pipeline" && <>{!authLoading && !isPro && <div style={{ background: '#F43F5E12', border: '1px solid #F43F5E30', borderRadius: 10, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}><div><span style={{ fontSize: 13, fontWeight: 700, color: '#F43F5E' }}>⏰ Trial Ended: </span><span style={{ fontSize: 13, color: T.s }}>Subscribe to keep your pipeline active</span></div><div onClick={() => startCheckout({ priceId: STRIPE_PRICES.recruiter, plan: 'recruiter' })} style={{ padding: '8px 16px', borderRadius: 8, background: T.a, color: '#000', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Subscribe Now →</div></div>}<Pipeline leads={leads} onSelectLead={handleSelectLead} onNavigate={setViewWithHistory} askRueInline={askRueInline} search={search} setSearch={setSearch} onTriggerDraftEmail={() => setAutoDraftEmail(true)} userId={effectiveUserId} userProfile={effectiveProfile} /></>}
        {view === "crm" && <>{!authLoading && !isPro && <div style={{ background: '#F43F5E12', border: '1px solid #F43F5E30', borderRadius: 10, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}><div><span style={{ fontSize: 13, fontWeight: 700, color: '#F43F5E' }}>⏰ Trial Ended: </span><span style={{ fontSize: 13, color: T.s }}>Subscribe to keep your pipeline active</span></div><div onClick={() => startCheckout({ priceId: STRIPE_PRICES.recruiter, plan: 'recruiter' })} style={{ padding: '8px 16px', borderRadius: 8, background: T.a, color: '#000', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Subscribe Now →</div></div>}<CRM leads={leads} onSelectLead={handleSelectLead} onNavigate={setViewWithHistory} askRueInline={askRueInline} userId={effectiveUserId} profile={effectiveProfile} onBulkDelete={(ids) => setLeads(p => p.filter(l => !ids.includes(l.id)))} /></>}
