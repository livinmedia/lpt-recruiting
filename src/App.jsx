...
export default function Livi(){
  ...
  const AskLiviBar=({prompts})=>(
    <>
      ...
      {inlineResponse&&!inlineLoading&&<div style={{marginBottom:20,padding:"20px 24px",borderRadius:10,background:T.as,border:`1px solid ${T.a}20`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:12,color:T.a,fontWeight:700,letterSpacing:1.5}}>🤖 LIVI RESPONSE</span><div style={{display:"flex",gap:12}}><span onClick={()=>{navigator.clipboard?.writeText(inlineResponse)}} style={{fontSize:12,color:T.s,cursor:"pointer"}}>📋 Copy</span><span onClick={()=>setInlineResponse(null)} style={{fontSize:12,color:T.s,cursor:"pointer"}}>✕ Close</span></div></div><pre style={{fontSize:14,color:T.t,lineHeight:1.7,whiteSpace:"pre-wrap",fontFamily:"inherit",margin:0,maxHeight:400,overflow:"auto"}}>{inlineResponse}</pre></div>}
    </>
  );
