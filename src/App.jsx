...
<div style={{marginBottom:24}}>
  <div style={{fontSize:18,fontWeight:800,color:T.t,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
    <span style={{width:4,height:20,borderRadius:2,background:T.a,display:"inline-block"}}></span>Newest Leads
  </div>
  <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:8}} className="hide-scrollbar">
    {recentLeads.map(l=>(
      <div key={l.id} onClick={()=>{setSelLead(l);setViewWithHistory("lead");}} style={{minWidth:220,background:T.card,border:`1px solid ${T.b}`,borderRadius:12,padding:16,cursor:"pointer",flexShrink:0,transition:"border-color 0.2s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=T.a} onMouseLeave={e=>e.currentTarget.style.borderColor=T.b}>
        <div style={{fontSize:15,fontWeight:700,color:T.t,marginBottom:6}}>{l.first_name}{l.last_name?" "+l.last_name:""}</div>
        {l.brokerage&&<div style={{fontSize:12,color:T.s,marginBottom:4}}>🏢 {truncate(l.brokerage,22)}</div>}
        {l.source&&<div style={{fontSize:12,color:T.m,marginBottom:8}}>📥 {l.source}</div>}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
          <span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:(l.pipeline_stage==="recruited"?T.a:l.pipeline_stage==="pitched"?T.bl:l.pipeline_stage==="researched"?T.p:T.y)+"18",color:l.pipeline_stage==="recruited"?T.a:l.pipeline_stage==="pitched"?T.bl:l.pipeline_stage==="researched"?T.p:T.y,fontWeight:700,textTransform:"capitalize"}}>{l.pipeline_stage||"new"}</span>
          <span style={{fontSize:11,color:T.m}}>{ago(l.created_at)}</span>
        </div>
      </div>
    ))}
  </div>
