  ...
  return(
    <div style={{flex:1,overflow:"auto",padding:"24px 32px"}}>
      ...
      <div className="two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:24}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12}}>{[["TIER",lead.tier,T.p],["URGENCY",lead.urgency,{HIGH:T.r,MEDIUM:T.y,LOW:T.a}[lead.urgency]||T.s],["TREND",lead.trend||"—",T.bl]].map(([l,v,c])=><div key={l} style={{background:T.card,borderRadius:10,padding:"16px",border:`1px solid ${T.b}`,textAlign:"center"}}><div style={{fontSize:11,color:T.m,letterSpacing:2,marginBottom:4}}>{l}</div><div style={{fontSize:20,fontWeight:800,color:c}}>{v||"—"}</div></div>)}
          {(()=>{const s=lead.interest_score||0;const lvl=s<=20?["Cold",T.m]:s<=40?["Warming",T.bl]:s<=60?["Interested",T.y]:s<=80?["Hot","#f97316"]:["On Fire 🔥",T.r];return(
            <div style={{background:T.card,borderRadius:10,padding:"16px",border:`1px solid ${T.b}`,textAlign:"center"}}>
              <div style={{fontSize:11,color:T.m,letterSpacing:2,marginBottom:4}}>INTEREST</div>
              <div style={{fontSize:20,fontWeight:800,color:lvl[1]}}>{s}</div>
              <div style={{fontSize:11,fontWeight:600,color:lvl[1],marginTop:2}}>{lvl[0]}</div>
              <div style={{height:4,borderRadius:2,background:T.d,marginTop:6,overflow:"hidden"}}><div style={{height:"100%",borderRadius:2,background:lvl[1],width:`${s}%`,transition:"width 0.3s"}}/></div>
            </div>);})()}
          </div>
          {lead.activity_summary&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{lead.activity_summary.opens!=null&&<div style={{padding:"4px 10px",borderRadius:6,background:T.bl+"12",color:T.bl,fontSize:12,fontWeight:600}}>📧 {lead.activity_summary.opens} opens</div>}{lead.activity_summary.clicks!=null&&<div style={{padding:"4px 10px",borderRadius:6,background:T.p+"12",color:T.p,fontSize:12,fontWeight:600}}>🖱️ {lead.activity_summary.clicks} clicks</div>}{lead.activity_summary.visits!=null&&<div style={{padding:"4px 10px",borderRadius:6,background:T.a+"12",color:T.a,fontSize:12,fontWeight:600}}>👁️ {lead.activity_summary.visits} visits</div>}</div>}
  ...
  return(
    <>
      ...
      {userId&&(()=>{
        ...
        const needsCompeting=sharePage==="calculator"||sharePage==="why-switch"||sharePage==="join";
        const compParam=needsCompeting&&competingBrokerage?`&target=${encodeURIComponent(competingBrokerage)}`:targetParam;
        const compIds=new Set(["calculator","why-switch","join"]);
        const sel=sharePage?[...PAGES.map(p=>({...p,url:`https://rkrt.in/${p.path}${trackingRef}${compIds.has(p.id)?compParam:targetParam}`})),...blogPages,...filteredPosts].find(p=>p.id===sharePage):null;
        return(
